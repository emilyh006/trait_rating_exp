...
dat <- dat %>%
  mutate(
    ...
    male_record = ifelse(male_politician_image %in% clean_male_images, 0, 1))


# Reorder the columns for better organization and easier readability (personal preference)
dat <- dat %>%
  select(id, male_politician_image, male_record, trait, ratings, rt, corruptible_imageorder, genuine_imageorder, attractive_imageorder, dominant_imageorder)
