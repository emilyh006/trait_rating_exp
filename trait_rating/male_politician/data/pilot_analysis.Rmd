...
To test H1a, we will conduct an aggregate-level t-test for the rating of corruptibility. We will first compute the average corruptibility rating for a given politician across all participants. We will then test whether politicians with corruption records are rated more
corruptible on average than politicians with clean records using a two-sample one-sided t-test.
...
male_info_data = read.csv("male.csv")


# Standardize the index columns of both dataframe:
avg_ratings_across_politician <- avg_ratings_across_politician %>%
  mutate(
...
male_info_data <- male_info_data %>%
  mutate(
    image_name = str_trim(tolower(image_name)),
...
merged_data <- left_join(
  avg_ratings_across_politician,
  male_info_data,
  by = c("male_politician_image" = "image_name")
)
...
```{r}
# Function to perform logistic regression for a specific trait
```
...
run_regression <- function(trait) {
  ...
  # Filter the dataset for the specified trait
  data_filtered <- merged_data %>%
    filter(trait == !!trait)  # Filter rows where `trait` matches the input
  
  
  # Fit the logistic regression model
  model <- glm(corruption_record ~ avg_rating + age + glasses + beard + mustache + pixels 
               + AU06 + AU12 + happiness, data = data_filtered, family = "binomial")
  
  # ask Yufei what about color of hair and clothing, as well as the source of the photos (whether source is used in female politicians)
...
```{r}
  ...
  # Fit the logistic regression model
  model <- glm(corruption_record ~ avg_rating + trait + age + glasses + beard + mustache + pixels 
               + AU06 + AU12 + happiness, data = merged_data, family = "binomial")
...
wide_data <- merged_data %>%
  pivot_wider(
    names_from = trait,      # Create new columns from `trait`
    values_from = avg_rating # Fill these columns with `avg_rating`
  )
```
...
```{r}
  ...
  # Fit the logistic regression model
  #model <- glm(corruption_record ~Attractive+ Corruptible+ Dominant+Genuine + age + glasses + beard + mustache + pixels 
   #            + AU06 + AU12 + happiness, data = wide_data, family = "binomial")

print(cor.test(wide_data$Corruptible,wide_data$Attractive))
print(cor.test(wide_data$Corruptible,wide_data$Dominant))
print(cor.test(wide_data$Corruptible,wide_data$Genuine))
